...
mob
	...
	proc
		...
		pixel_move(dpx, dpy)
				...
				if(t.pleft != t.pright)
					...
					if(px + pwidth < t.px)
						if(t.py + t.pleft > py + 3)
							...
							if(trace) trace.event("[world.time]: pixel_move calling bump: t = [t] (\ref[t]), d = RIGHT ([RIGHT])")
							bump(t, RIGHT)
							continue
					if(px > t.px + t.pwidth)
						if(t.py + t.pright > py + 3)
							...
							if(trace) trace.event("[world.time]: pixel_move calling bump: t = [t] (\ref[t]), d = LEFT ([LEFT])")
							bump(t, LEFT)
							continue
					...
					if(py + dpy < h)
						if(py + pheight < t.py)
							...
							if(trace) trace.event("[world.time]: pixel_move calling bump: t = [t] (\ref[t]), d = UP ([UP])")
							bump(t, UP)
						else
							// py = h
							dpy = h - py

							if(trace) trace.event("[world.time]: pixel_move calling bump: t = [t] (\ref[t]), d = DOWN ([DOWN])")
							bump(t, DOWN)
			...
			if(trace) trace.event("[world.time]: end pixel_move: dpx = [dpx], dpy = [dpy]")
		...
		set_pos(nx,ny, map_z = -1)
			...
			if(moved)
				...
				pixel_x = px - x * icon_width + offset_x
				pixel_y = py - y * icon_height + offset_y
